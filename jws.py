...
def autodetect_backend(audio_file, param):
    if program_exists("mpg123"):
        external_backend(audio_file, "mpg123 %s >/dev/null 2>&1")
        return
        
    if program_exists("playsound"):
        external_backend(audio_file, "playsound %s >/dev/null 2>&1")
        return
...
def main():
    ...
    data = {
        'tl': options.language,
        'q': (u' '.join([i.decode('utf-8') for i in phrases]) or u'nada não').encode('utf-8'),
    }
